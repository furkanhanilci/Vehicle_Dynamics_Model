...
def get_simulation(x=0, vx=10, y=0, vy=0, z=0.715, vz=0, r=0, vr=0, p=0, vp=0, yaw=0, vyaw=0, T=1200., amplitude=0.1,
    ...
    :sim_dt:    Time step used in the simulation, assumed to be 1 ms
    :state:     Array representing the initial state of the vehicle, including position, velocity, and orientation.
    :controls:  Array storing the control inputs for each time step, including throttle and steering angle.
    :t:         Array representing time points throughout the simulation, generated using np.linspace.
    :delta:     Array representing the sinusoidal steering angle, calculated based on amplitude, frequency, and t.
    :mu:        Array representing the friction coefficients for the vehicle's wheels.
    :_run:      Array storing the simulation results, including vehicle state variables such as position, velocity, and orientation.
